...
- JSX를 직접 반환하는 `.stories.tsx` 파일에서 IDE가 `React refers to a UMD global` 오류를 내면 `import React from "react";`를 추가
- Story에서 `render`를 사용하더라도 필수 props가 있으면 `args`를 함께 선언
...
`render`를 사용하는 Story라도, 컴포넌트에 필수 props가 있으면 `args`를 함께 선언합니다.
예: `avatars`, `children`, `items` 같은 필수 데이터 props

`StoryObj<typeof meta>` 기준으로 필수 props가 `args`에 없으면
`Property 'args' is missing` 타입 오류가 발생할 수 있습니다.
...
`UsageExample`도 `render`만 단독으로 쓰지 않습니다.
컴포넌트에 필수 props가 있으면 최소 동작 가능한 기본 `args`를 반드시 선언합니다.
